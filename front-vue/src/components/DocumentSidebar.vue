<template>
  <section class="flex h-full min-h-0 flex-col overflow-hidden px-6 py-6 max-[720px]:px-[18px] max-[720px]:py-[18px]">
    <div class="flex items-start justify-between gap-4">
      <div class="min-w-0">
        <p class="text-xs font-bold uppercase tracking-[0.12em] text-ink-500">文档集合</p>
        <h2 class="mt-1 text-lg font-semibold text-ink-950">浏览和下载知识库文档</h2>
      </div>
      <a-button
        type="text"
        shape="circle"
    ...
    <div v-if="authenticated" class="mt-4 flex flex-wrap items-center gap-2">
      ...
      <a-button :loading="documentsLoading" :disabled="uploading" @click="emit('refresh-documents')">
        <template #icon>
          <ReloadOutlined />
        </template>
        刷新
    ...
    <!-- Upload progress -->
    <div v-if="uploading && fileUploads.length > 0" class="mt-4 border border-ink-300 bg-white px-4 py-3">
      <div class="flex items-center justify-between gap-2">
        <span class="text-sm font-medium text-ink-900">
          <span v-if="fileUploads.length > 1" class="mr-2 text-ink-500">
            ({{ fileUploads.filter(f => f.status === 'complete').length }}/{{ fileUploads.length }})
          </span>
          上传进度
        </span>
        <a-button
      ...
      <div class="mt-2 max-h-[260px] space-y-2 overflow-y-auto">
        <div v-for="(entry, i) in fileUploads" :key="`${entry.filename}-${i}`">
          <div class="flex items-center gap-1.5 text-xs">
            <LoadingOutlined v-if="entry.status === 'uploading'" class="text-accent-500" />
            <CheckCircleOutlined v-else-if="entry.status === 'complete'" class="text-emerald-600" />
            <CloseCircleOutlined v-else-if="entry.status === 'error'" class="text-rose-600" />
            <span class="truncate text-ink-900" :title="entry.filename">{{ entry.filename }}</span>
            <span v-if="entry.status === 'error' && entry.errorMessage" class="text-rose-600">({{ entry.errorMessage }})</span>
          </div>
          <a-progress
    ...
    <div class="mt-[18px] flex min-h-0 flex-1 flex-col border-t border-ink-950/8 pt-[18px]">
      ...
      <div v-if="documentsLoading" class="grid min-h-[180px] place-items-center">
        <a-spin />
      </div>
      <div v-else-if="documents.length === 0" class="grid min-h-[180px] place-items-center">
      ...
      <div v-else class="flex min-h-0 flex-1 flex-col overflow-auto pr-1">
        ...
        >
          ...
          <div class="flex shrink-0 flex-wrap items-center gap-2 max-[720px]:gap-1.5">
            <span class="inline-flex bg-ink-950/6 px-3 py-1 text-xs font-medium text-ink-700">
              {{ item.segmentCount }} 段
            </span>
            <a-button
              type="text"
            ...
            <a-button
              ...
              size="small"
              class="!min-h-[36px] !min-w-[36px] !text-ink-600 hover:!text-accent-500"
              @click="emit('show-download-link', item.documentId, item.filename)"
            >
              <template #icon>
                <DownloadOutlined />
              </template>
...
import type { DocumentListItem, FileUploadEntry, HealthState, UploadProgressEvent } from "../types";
